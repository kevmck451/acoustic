...
output_folder = f'{base_dir}/Spec Plot Graphs/combo levels'
frame_duration = 0.05

# Create MFCC Graphs
directory = Path(f'{af.basepath}/Synthetic/diesel_amb_mix_3')
save_path = f'{base_dir}/Spec Plot Graphs/combo levels/diesel amb'
Path(save_path).mkdir(exist_ok=True)
for file in directory.iterdir():
    if 'wav' in str(file):
        audio = Audio_Abstract(filepath=file, num_channels=1)
        audio.spec_plot, audio.spec_plot_fb = process.average_spectrum(audio, display=True, save=True, save_path=save_path)
...
save_path = f'{base_dir}/Spec Plot Graphs/combo levels/diesel hex'
Path(save_path).mkdir(exist_ok=True)
for file in directory.iterdir():
    if 'wav' in str(file):
        audio = Audio_Abstract(filepath=file, num_channels=1)
